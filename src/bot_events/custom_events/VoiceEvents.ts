import { ChannelType, Client, GuildMember, PermissionFlagsBits, TextChannel, VoiceChannel, VoiceState } from "discord.js";
...
export default class VoiceEvents implements CustomEvent{
    ...
    async createCustomVoiceChannel(member:VoiceState, client:Client){
        ...
        const info_channel = member.guild.channels.cache.get(guild_data.infoVoiceChannel) as TextChannel
        ...
        threed_channel.send(`Made a new voice chat thread for you! <@!${member.id}>`)
    ...
    async removeCustomVoiceChannel(channelId:string, member:GuildMember, client:Client){
        ...
        if(voice_channel.members.size <= 0) {
            ...
            const new_owner_profile = await GamerBotAPIInstance.models.get_profile_data(new_owner.id);
            new_owner_profile.privateVoiceID = voice_channel.id;
            new_owner_profile.privateVoiceThreadID = profile_data.privateVoiceThreadID;
            await new_owner_profile.save();
            profile_data.privateVoiceID = "";
            profile_data.privateVoiceThreadID = "";
            await profile_data.save();
            await threed_channel?.members.add(new_owner.id);
            await threed_channel?.send(`__**${member.user.username}**__ har lämnat röstkanalen. Röstkanalens nya ägare är nu __**${member.user.username}**__`)
        }
    }
}