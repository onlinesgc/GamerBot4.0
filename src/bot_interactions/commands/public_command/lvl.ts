...
export default class LvlCommand implements Command {
    ...
    async execute (interaction: CommandInteraction, profileData: PorfileData){
        let user = interaction.options.get("user", false)?.user || interaction.user;
        if(profileData.userID != user.id)
            profileData = await GamerBotAPIInstance.models.get_profile_data(user.id as string);
        const file = new AttachmentBuilder(await GamerBotAPIInstance.models.get_user_frame(user.id as string ,interaction.member?.user.username as string,interaction.user.avatarURL({extension:"png"}) as string) as any);
