...
export default class ColorCommand implements Command {
    ...
    async execute(interaction: CommandInteraction, profileData: PorfileData) {
        ...
        if(!(interaction.member.roles as GuildMemberRoleManager).cache.hasAny(...ROLES_PERMISSION)){
            interaction.editReply({content: "Du har inte tillräckligt hög level för att ändra din färg!"})
            return;
        }
        ...
        await (interaction.member.roles as GuildMemberRoleManager).remove(rolesArray);
