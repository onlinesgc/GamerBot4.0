...
export default class TopicCommand implements Command {
    ...
    async execute(interaction: CommandInteraction){
        ...
        if(topic!=undefined){
            ...
            if(guild_config.topicList.length == 0) return interaction.editReply("Det finns inga topics i systemet! Lägg till några :)")
            await interaction.editReply(guild_config.topicList[await getRndInteger(0,guild_config.topicList.length)])
        }
    }
