...
const SVEROK_ID = "1016685055357222942";
const CHECKIN_ID = "1437906000173727794";
...
export default class CheckinCommand implements Command {
    ...
    async execute(interaction: CommandInteraction) {
        ...
        if (
            ...
            interaction.reply({
                content:
                    "Du är inte föreningsmedlem, eller har inte rollen i discord. Bli medlem på https://blimedlem.sgc.se/ , bli medlem och gör sedan kommandot /sverok för att bekräfta ditt medlemskap i discord",
                ephemeral: true,
            });
            return;
        }
        ...
        if (
            ...
            interaction.reply({
                content: "Du har redan checkat in",
                ephemeral: true,
            });
            return;
        }
        ...
        if (modal == null) {
            interaction.reply({
                content: "Tiden för att svara har gått ut",
                ephemeral: true,
            });
            return;
        }
        const firstname = modal.fields
            .getTextInputValue("firstname")
            .toLowerCase();
        const lastname = modal.fields
        ...
        if (!data.response.member_found) {
            modal.reply({
                content:
                    "Ditt namn matchade inte med något medlemskap, försök igen",
                ephemeral: true,
            });
            return;
        }
        ...
        if (
            checkin_data.find(
                (checkin) => checkin.name == `${firstname} ${lastname}`,
            )
        ) {
            modal.reply({
                content: "Denna person har redan checkat in",
                ephemeral: true,
            });
            return;
        }
        ...
        modal.reply({
            content: "Du är nu incheckad i årsmötet",
            ephemeral: true,
        });
