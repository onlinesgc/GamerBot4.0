...
export default class GiveFrameCommand implements Command {
    ...
    async execute(interaction: CommandInteraction) {
        ...
        const frame_config = (await GamerBotAPIInstance.models.get_guild_data("516605157795037185")).frameConfig;
        //eslint-disable-next-line
        const frame_data = frame_config.find((f:any) => f.id.toString() === frame);
        ...
        if (action === "add") {
            ...
            //eslint-disable-next-line
            profile_data.exclusiveFrames = profile_data.exclusiveFrames.map((f:any) => f.toString());
            profile_data.exclusiveFrames = profile_data.exclusiveFrames.filter((f) => f !== exclusive_frame_id);
        }
        profile_data.save();
    ...
    async autoComplete(interaction: AutocompleteInteraction) {
        ...
        let choises = await getAllFrames("516605157795037185",interaction.client as GamerbotClient);
        choises = choises.slice(10,choises.length);
        const filtered = choises.filter((choise) => choise.name.includes(focusedValue.value));
