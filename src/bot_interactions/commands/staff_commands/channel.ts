import { SlashCommandBuilder, CommandInteraction, PermissionFlagsBits, EmbedBuilder, GuildChannel, Guild } from "discord.js";
...
export default class ChannelCommand implements Command {
    ...
    data = new SlashCommandBuilder()
        ...
        .addChannelOption((option) => option.setName("channel").setDescription("Kanalen du vill ändra på").setRequired(true))
        .addStringOption((option) => option.setName("type").setDescription("Typ av ändring").setRequired(true).addChoices(
        ...
        .addStringOption((option) => option.setName("parent").setDescription("[använd bara med relocate] Den kategorin du vill flytta till").setRequired(false));
    async execute (interaction: CommandInteraction){
        ...
        switch (type) {
            case "relocate" :{
                if (!parent) {
                    channel_embed.setTitle("Du måste ange en kategori att flytta till!")
