import { SlashCommandBuilder, CommandInteraction, PermissionFlagsBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder } from "discord.js";
...
export default class ModLogCommand implements Command {
    ...
    data = new SlashCommandBuilder()
        ...
        .addUserOption((option) => option.setName("user").setDescription("Personen du vill se loggar för").setRequired(true));
    async execute (interaction: CommandInteraction){
        const user = interaction.options.get("user", true).user;
        if(!user) return interaction.editReply("Användaren finns inte");

        const modlogs = (await GamerBotAPIInstance.models.get_profile_data(user.id)).modLogs;

        //eslint-disable-next-line
        if(!modlogs && !Array.isArray(modlogs) && (modlogs as Array<any>).length < 1) return interaction.editReply("Användaren har inga loggar");


        const modlog_rows = new ActionRowBuilder<ButtonBuilder>()
            .addComponents(
                [
                    new ButtonBuilder().setCustomId("modlog_previous").setEmoji("⬅️").setStyle(ButtonStyle.Secondary).setDisabled(true),
                    new ButtonBuilder().setCustomId("modlog_next").setEmoji("➡️").setStyle(ButtonStyle.Secondary)
                ]
            );
        ;
        ...
        if(amount_logs_shown === 0) return interaction.editReply("Användaren har inga loggar");
        ...
        const message = await interaction.editReply({embeds: [await this.getUserModLogs(modlogs,interaction,start_pointer,log_count)],components: [modlog_rows]});
        ...
        collector.on("collect", async (button_interaction) => {
            if(button_interaction.customId === "modlog_previous"){
                ...
                if(start_pointer <= 0){
                    start_pointer = 0;
                    (modlog_rows.components[0] as ButtonBuilder).setDisabled(true);
                }
            ...
            await button_interaction.update({embeds: [await this.getUserModLogs(modlogs,interaction,start_pointer,log_count)],components: [modlog_rows]});
        });
