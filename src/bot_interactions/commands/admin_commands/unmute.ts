...
export default class UnMuteCommand implements Command {
    ...
    data = new SlashCommandBuilder()
        ...
        .addUserOption((option) => option.setName("user").setDescription("Personen du vill unmuta").setRequired(true))
        .addStringOption((option) => option.setName("reason").setDescription("Anledning till unmutet").setRequired(true));
    async execute (interaction: CommandInteraction){
        const member = interaction.options.get("user", true).member as GuildMember;
        const reason = interaction.options.get("reason", true).value as string;

        if(!member) return interaction.editReply("Användaren finns inte");

        const has_sent_message = await this.unMute(member, reason, interaction.user.id);
    ...
    async unMute(member: GuildMember, reason:string, authorId:string){
        ...
        const modlog = new ModLog("unmute",member.id,member.user.username,reason,null,Date.now(),authorId);
        ...
        await member.send(`Du har blivit unmutad i SGC.\nAnledningen är **${reason}**`).catch(() => has_sent_message = false);
